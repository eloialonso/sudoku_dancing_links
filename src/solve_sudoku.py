...
def solve_sudoku(*, grid: Optional[List[List[int]]] = None, path: Optional[Path|str] = None) -> None:
    ...
    for i, (r, c, n) in enumerate(possibilities):
        ...
        cell_constraints = [f'R{r+1}C{c+1}', f'R{r+1}#{n+1}', f'C{c+1}#{n+1}', f'B{b+1}#{n+1}']
    ...
    def format_solution(sol):
        ...
        return get_board_fmt().format(*(green(y) if x == 0 else x for x, y in zip(chain(*grid), chain(*sol_grid))))
